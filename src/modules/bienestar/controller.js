...
export const getCheckinHoy = async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ ok: false, error: "No hay check-in hoy" });
...
export const getEstadisticas = async (req, res) => {
  try {
    ...
    res.json({ ok: true, data: { ...rows[0], tendencia: tendencia.rows } });
  } catch (err) { manejarError(res, err); }
};
...
export const getDiario = async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ ok: false, error: "Entrada no encontrada" });
    res.json({ ok: true, data: rows[0] });
  } catch (err) { manejarError(res, err); }
};
...
export const deleteDiario = async (req, res) => {
  try {
    ...
    if (!rowCount) return res.status(404).json({ ok: false, error: "Entrada no encontrada" });
...
export const getDiarioAleatorio = async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ ok: false, error: "No hay entradas en el diario" });
    res.json({ ok: true, data: rows[0] });
  } catch (err) { manejarError(res, err); }
};
...
export const getInsights = async (req, res) => {
  try {
    ...
    if (desdeCache.rows.length) return res.json({ ok: true, data: desdeCache.rows[0].resultado });
...
export const completarPausa = async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ ok: false, error: "Pausa no encontrada" });
    res.json({ ok: true, data: rows[0] });
  } catch (err) { manejarError(res, err); }
};
