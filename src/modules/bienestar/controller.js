...
export const createCheckin = async (req, res) => {
  const { usuario_id, estado_animo, energia, sueno_horas, notas } = req.body;
  const { rows } = await pool.query(
    `INSERT INTO checkins_emocionales (usuario_id, estado_animo, energia, sueno_horas, notas)
     ...
     RETURNING *`,
    [usuario_id, estado_animo, energia, sueno_horas, notas]
  );
...
export const createDiario = async (req, res) => {
  const { usuario_id, titulo, contenido, etiquetas, es_publico } = req.body;
  const { rows } = await pool.query(
    `INSERT INTO diario_personal (usuario_id, titulo, contenido, etiquetas, es_publico)
     VALUES ($1, $2, $3, $4, $5) RETURNING *`,
    [usuario_id, titulo, contenido, etiquetas || [], es_publico || false]
  );
  res.status(201).json(rows[0]);
};
...
export const listDiario = async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT * FROM diario_personal WHERE usuario_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
    [usuario_id, limite, offset]
  );
...
export const getDiario = async (req, res) => {
  const { id } = req.params;
  const { rows } = await pool.query(`SELECT * FROM diario_personal WHERE id = $1`, [id]);
...
export const updateDiario = async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [id, titulo, contenido, etiquetas, es_publico]
  );
  if (!rows.length) return res.status(404).json({ error: "Entrada no encontrada" });
  res.json(rows[0]);
};
...
export const deleteDiario = async (req, res) => {
  const { id } = req.params;
  const { rowCount } = await pool.query(`DELETE FROM diario_personal WHERE id = $1`, [id]);
...
export const getInsights = async (req, res) => {
  ...
  const desdeCache = await pool.query(
    `SELECT resultado FROM analisis_ia WHERE usuario_id = $1 AND tipo = 'insights' AND cache_hasta > NOW() ORDER BY created_at DESC LIMIT 1`,
    [usuario_id]
  );
  ...
  await pool.query(
    ...
    [usuario_id, JSON.stringify(resultado)]
  );
  res.json(resultado);
};
...
export const programarPausa = async (req, res) => {
  const { usuario_id, ejercicio, duracion_minutos, programada_para } = req.body;
  const { rows } = await pool.query(
    `INSERT INTO pausas_activas (usuario_id, ejercicio, duracion_minutos, programada_para) VALUES ($1, $2, $3, $4) RETURNING *`,
    [usuario_id, ejercicio, duracion_minutos, programada_para]
  );
  res.status(201).json(rows[0]);
};
...
export const completarPausa = async (req, res) => {
  const { id } = req.body;
  const { rows } = await pool.query(
    `UPDATE pausas_activas SET completada = true WHERE id = $1 RETURNING *`,
    [id]
  );
